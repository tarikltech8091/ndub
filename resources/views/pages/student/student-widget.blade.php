		...
		<div class="panel panel-danger">
			...
			<div id="collapseTwo" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingTwo">
				<div class="panel-body panel_text today_schedule_list">
					<?php 
					$student_notice_board =\App\notice::AllNoticeInfo();
					?>

					@if(!empty($student_notice_board))
					<marquee  direction="up" scrollamount="3" onMouseOver="this.setAttribute('scrollamount', 0, 0);" OnMouseOut="this.setAttribute('scrollamount', 3, 0);">
						<ul>
							...
							@foreach($student_notice_board as $key => $list)
							<li class="alert alert-success">
								<p style="font-size:12px;">
									<i class="fa fa-calendar-o"></i>{{$list->created_at}}&nbsp;
									<span style="color:#F50A19;"> Form:{{$list->notice_from_type}}</span>
								</p>
								<a style="font-weight:bold; text-decoration:none; margin-left:15px;" data-toggle="modal" data-target="#studentnoticeModal"  data-id="{{$list->notice_tran_code}}" class="text_none student_notice_show" href="">{{str_limit($list->notice_subject, 20)}}</a>
							</li>
							@endforeach
					...
					<div class="more_btn">
						<a href="{{url('/student/all/notice')}}" class="btn btn-primary" data-toggle="tooltip" title="View All Notice" >View All</a>
					</div>
					@else
